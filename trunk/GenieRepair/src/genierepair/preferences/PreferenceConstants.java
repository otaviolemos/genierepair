package genierepair.preferences;

/**
 * Constant definitions for plug-in preferences
 */
public class PreferenceConstants {

	public static final String RELATED_WORD_SERVER = "relatedWordServer";
	public static final String SLICE_SERVER = "sliceServer";
	public static final String FILE_SERVER = "fileServer";
	public static final String SOLR_SERVER = "solrServer";
	public static final String LISTEN_JUNIT = "listenJUnit";
	public static final String ANNOTATIONPACKAGE = "annpackage";
	public static final String ANNOTATIONCLASS = "annclass";
	
	
	
}
